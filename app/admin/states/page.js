...
const fetchStates = async (page = 1) => {
  ...
  // Fetch FAQs for a state
const fetchFAQs = async (stateId) => {
  setLoadingFAQs(true);
  try {
    const res = await fetch(`/api/state-faqs?state_id=${stateId}`);
    const data = await res.json();
    setFaqs(data.data || []);
  } catch (error) {
...
const handleFAQSubmit = async (e) => {
  ...
  try {
    ...
    if (res.ok) {
      ...
      setEditingFAQ(null);
      setFaqFormData({
        state_id: faqFormData.state_id,
        question: '',
        answer: '',
        serial_no: 0
      });
      fetchFAQs(parseInt(faqFormData.state_id));
    } else {
      const error = await res.json();
      alert('Error: ' + (error.error || 'Something went wrong'));
    }
...
const handleDeleteFAQ = async (id) => {
  ...
  try {
    const res = await fetch(`/api/state-faqs?id=${id}`, {
      method: 'DELETE'
    });
  ...
  >
    ...
    {countries.map((country) => (
      ...
      {/* FAQ Modal */}
{showFAQModal && editingState && (
  <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
    <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
      <div className="sticky top-0 bg-white border-b px-6 py-4 flex justify-between items-center">
        <div>
          <h2 className="text-xl font-bold text-gray-900">Manage FAQs</h2>
          <p className="text-sm text-gray-500">For: {editingState.name}</p>
        </div>
        <button
              ...
              {editingFAQ && (
                <button
                  ...
                  onClick={() => {
                    setEditingFAQ(null);
                    setFaqFormData({
                      state_id: String(editingState.state_id),
                      question: '',
                      answer: '',
                      serial_no: 0
                    });
