...
export async function getCountries() {
  const sql = 'SELECT id, name, web_slug, digital_slug, gmb_slug, content_slug, seo_slug, app_slug, status FROM global_countries WHERE status = 1 ORDER BY name ASC';
  return await query(sql);
}

export async function getStatesByCountry(countryId) {
  const sql = 'SELECT state_id as id, name, country_id, web_slug, digital_slug, gmb_slug, content_slug, seo_slug, app_slug, status FROM global_states WHERE country_id = ? AND status = 1 ORDER BY name ASC';
  return await query(sql, [countryId]);
}

export async function getCitiesByState(stateId) {
  const sql = 'SELECT city_id as id, city as name, state_id, web_slug, digital_slug, gmb_slug, content_slug, seo_slug, app_slug, status FROM global_cities WHERE state_id = ? AND status = 1 ORDER BY city ASC';
...
export async function getCityDetails(citySlug) {
  const sql = `
    SELECT c.*, 
           ...
           s.web_slug as state_web_slug, s.seo_slug as state_seo_slug, s.app_slug as state_app_slug, s.gmb_slug as state_gmb_slug, s.digital_slug as state_digital_slug,
           co.name as country_name, co.id as country_id,
           co.web_slug as country_web_slug, co.seo_slug as country_seo_slug, co.app_slug as country_app_slug, co.gmb_slug as country_gmb_slug, co.digital_slug as country_digital_slug
    FROM global_cities c
    JOIN global_states s ON c.state_id = s.state_id
    JOIN global_countries co ON s.country_id = co.id
    WHERE c.web_slug = ? OR c.gmb_slug = ? OR c.seo_slug = ? OR c.content_slug = ? OR c.digital_slug = ? OR c.app_slug = ?
  `;
...
export async function getStateDetails(stateSlug) {
  const sql = `
    ...
    JOIN global_countries co ON s.country_id = co.id
    WHERE s.web_slug = ? OR s.gmb_slug = ? OR s.seo_slug = ? OR s.content_slug = ? OR s.digital_slug = ? OR s.app_slug = ?
  `;
...
export async function getCountryDetails(countrySlug) {
  const sql = 'SELECT * FROM global_countries WHERE web_slug = ? OR gmb_slug = ? OR seo_slug = ? OR content_slug = ? OR digital_slug = ? OR app_slug = ?';
  const results = await query(sql, [countrySlug, countrySlug, countrySlug, countrySlug, countrySlug, countrySlug]);
  return results[0] || null;
}
